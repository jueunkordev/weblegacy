...
public class HomeController {
	...
	public String joinok(@ModelAttribute membership_DTO dto, Model m) {
		...
		try {
			...
			if (result > 0) {
				m.addAttribute("msg", "alert('정상적으로 회원가입이 완료 되었습니다.'); " + "location.href='./kakao2.jsp';");
			} else {
	...
	/*
	 * ServletRequest, ServletResponse => session 불가능 HttpServletRequest,
	...
	public String web_loginok(@RequestParam(name = "code") String code,
			...
			@RequestParam(name = "kakao_nicknm", required = false) String kakao_nicknm, Model m, HttpServletRequest req)
			throws Exception {
		...
		if (code.equals("1")) { // 일반로그인 처리
			...
			if (all.size() > 0) {
				...
				msg = "alert('카카오 사용자로 로그인시 간편회원가입이 필요합니다.');" + "sessionStorage.setItem('mid','" + kakao_id + "');"
						+ "sessionStorage.setItem('mnick','" + kakao_nicknm + "');" + "location.href='../join.jsp';";
			}
			m.addAttribute("msg", msg);
		}
		return "joinok";
